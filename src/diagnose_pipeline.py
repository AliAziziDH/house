...
print(
    f"  Train Log Mean: {y_train_log.mean():.4f} | Sub Log Mean: {sub_log.mean():.4f}"
)
...
for col, tr_min, tr_max, te_min, te_max in out_of_bounds[:15]:
    print(
        f"  {col:20s}: Train [{tr_min:.2f}, {tr_max:.2f}] | Test [{te_min:.2f}, {te_max:.2f}]"
    )
...
pipe_ridge = make_pipeline(StandardScaler(), Ridge(alpha=15.0)).fit(
    X_train, y_train_log
)
coefs_ridge = pipe_ridge.named_steps["ridge"].coef_

pipe_lasso = make_pipeline(RobustScaler(), LassoCV(cv=5, random_state=42)).fit(
    X_train, y_train_log
)
coefs_lasso = pipe_lasso.named_steps["lassocv"].coef_

print(
    f"Ridge Coefs  -> Max: {coefs_ridge.max():.4f}, Min: {coefs_ridge.min():.4f}, Abs Mean: {np.abs(coefs_ridge).mean():.4f}"
)
print(
    f"Lasso Coefs  -> Max: {coefs_lasso.max():.4f}, Min: {coefs_lasso.min():.4f}, Non-zero: {(coefs_lasso != 0).sum()}/{len(coefs_lasso)}"
)
...
if len(preds_dict) > 1:
    ...
    for name in preds_dict:
        if name != "Final Ensemble":
            mape = (
                np.mean(
                    np.abs(preds_dict[name] - preds_dict["Final Ensemble"])
                    / preds_dict["Final Ensemble"]
                )
                * 100
            )
