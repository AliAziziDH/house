...
def preprocess_house_prices_data(data_dir: str = "./data") -> tuple:
    ...
    if not train_path.exists():
        alt_path = Path("/Users/out/.gemini/antigravity/scratch/house-prices-kaggle/data/train.csv")
        if alt_path.exists():
            train_path = alt_path
            test_path = Path("/Users/out/.gemini/antigravity/scratch/house-prices-kaggle/data/test.csv")
    ...
    for col in ord_cols:
        combined[col] = combined[col].fillna('None').map(QUALITY_MAP).fillna(0).astype(int)
