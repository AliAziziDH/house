...
X_test["LotFrontage"] = X_test.apply(
    lambda row: (
        neighborhood_median.get(row["Neighborhood"], global_median)
...
X_train["GarageAge"] = np.where(
    X_train["GarageYrBlt"] == 0, 0, X_train["YrSold"] - X_train["GarageYrBlt"]
)
X_test["GarageAge"] = np.where(
    X_test["GarageYrBlt"] == 0, 0, X_test["YrSold"] - X_test["GarageYrBlt"]
)

print("✅ Feature engineering completed.")
print(
    "   New features added: TotalSF, TotalPorchSF, TotalBathrooms, HouseAge, RemodAge, IsNew, QualityScore, GarageAge"
)
