...
col2.metric("Total Invested", f"${total_spend:,.0f}", f"{total_spend/budget*100:.1f}% of Budget")
col3.metric("Expected Profit", f"${total_profit:,.0f}")
col4.metric("Downside Risk", f"${total_downside:,.0f}", f"-{(total_downside/total_spend*100) if total_spend > 0 else 0:.1f}%")
...
with tab1:
    ...
    fig1.add_trace(go.Scatter(
        x=viz_df.index, y=viz_df["Price_Upper_Bound"],
    ...
    # Lower Bound
    fig1.add_trace(go.Scatter(
        x=viz_df.index, y=viz_df["Price_Lower_Bound"],
        mode='lines', line={"width": 0}, fill='tonexty', fillcolor='rgba(173, 216, 230, 0.3)',
        name='95% Conformal Envelope'
    ))

    # All Predictions
    fig1.add_trace(go.Scatter(
        x=viz_df.index, y=viz_df["SalePrice_pred"],
        mode='markers', marker={"size": 4, "color": 'gray', "opacity": 0.5},
    ...
    fig1.add_trace(go.Scatter(
        x=viz_df[selected_mask].index, y=viz_df[selected_mask]["SalePrice_pred"],
        mode='markers', marker={"size": 8, "color": 'red', "symbol": 'diamond'},
...
with tab3:
    ...
    display_cols = ["Id", "Selected_Fraction", "Asking_Price", "SalePrice_pred", "Expected_Profit", "Conformal_Downside"]
    st.dataframe(selected_df[display_cols].sort_values("Expected_Profit", ascending=False).style.format({
        "Selected_Fraction": "{:.2%}",
        "Asking_Price": "${:,.0f}",
        "SalePrice_pred": "${:,.0f}",
        "Expected_Profit": "${:,.0f}",
