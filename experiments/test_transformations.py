...
transformations = {
    ...
    "Box-Cox": lambda y: (
        ...
        .fit_transform(y.values.reshape(-1, 1))
        .flatten()
    ),
    "Yeo-Johnson": lambda y: (
...
inverse_transforms = {
    ...
    "Box-Cox": lambda y_pred: (
        PowerTransformer(method="box-cox")
        .inverse_transform(y_pred.reshape(-1, 1))
        .flatten()
    ),
    "Yeo-Johnson": lambda y_pred: (
        PowerTransformer(method="yeo-johnson")
        .inverse_transform(y_pred.reshape(-1, 1))
        .flatten()
    ),
...
for name, transform_func in transformations.items():
    ...
    for train_idx, val_idx in kf.split(X_train):
        ...
        y_train_fold, y_val_fold = (
            y_transformed.iloc[train_idx],
            y_transformed.iloc[val_idx],
        )
...
for name, metrics in results.items():
    print(
        f"   {name}: RMSE = {metrics['avg_rmse']:.4f} (+/- {metrics['std_rmse']:.4f})"
    )
